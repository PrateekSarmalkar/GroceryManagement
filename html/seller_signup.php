...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name) || empty($email) || empty($password) || empty($shop_name)) {
        ...
        try {
            ...
            $check = $pdo->prepare("SELECT * FROM sellers WHERE email = ?");
            $check->execute([$email]);
            ...
            if ($check->rowCount() > 0) {
                ...
                // Insert new seller
                $sql = "INSERT INTO sellers (name, email, password, phone, shop_name) VALUES (?, ?, ?, ?, ?)";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([$name, $email, $password, $phone, $shop_name]);
    ...
    <section class="signup">
        <div class="textbox">
            ...
            <form method="POST" action="">
                ...
                <button type="submit" class="sign_up_btn">SIGN UP AS SELLER</button>
            </form>
            <div class="divider">
                <span>OR</span>
            </div>
            <a href="signup.php" class="seller-signup-btn">SIGN UP AS CUSTOMER</a>
            <p class="account-text">Already have an account? 
                <a href="seller_signin.php">Sign In</a>
            </p>
        </div>
    </section>
</body>
